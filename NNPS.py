...
def load_targets(fname='bio-decagon-targets-all.csv'):
...
stitch2proteins_all = load_targets(fname='bio-decagon-targets-all.csv')
...
pca.fit(drug_label)
pca_= PCA(.95)
pca_.fit(drug_gene_adj)
drug_feat = np.concatenate((pca.transform(drug_label),pca_.transform(drug_gene_adj)),axis=1)
#------------------------------------------------------------------------
